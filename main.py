        ...
        while len(self.routes) > 0:
            ...
            if iteration % 20000 == 0:
                ...
                for i in self.completed_routes:
                    del i
                self.completed_routes = []
                print(f"     IT: {iteration} Route: {len(self.routes)} Removed: {self.completed_routes_len} Visited Node: {len(self.visited_nodes)} Last Node: {self.current_node.id}")


            iteration += 1

        print(f"\u001b[35;1m << ALL ROUTES: {len(self.routes)} ENDED: {len(self.completed_routes)} ARRIVED: {len(self.arrived_routes)} >> \u001b[0m")
...
#rs = PathFinder('data/4.txt', 1, 8)
rs = PathFinder('data/data.txt', 876500321, 1524235806)
rs.debug_node_id = 1092220329
rs.run()
